...
Use the existing `transition_evaluations` unit as the primary authorized budget: one charge per verified whole-turn execution class consumed. Extend receipts to account for all expanded transitions at the registered depth. Require both the same authorized budget and the same actual charged transition count for each paired search comparison; reject overruns and input drift. Record simulator work, evaluator calls, and wall-clock time as well. If a method cannot consume the exact paired allocation, mark that comparison unmatched and do not pool it as matched evidence.
