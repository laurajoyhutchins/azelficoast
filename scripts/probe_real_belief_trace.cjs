...
const {generatorVariants, mechanicsProjectionVariantCount} =
  createGeneratorPopulationSource({
    ...
    generatorCacheDir,
    actualCommit,
    generatorRounds: GENERATOR_ROUNDS,
...
const {opponentDistributionForSnapshot} = createOpponentPolicyEngine({
  Battle,
  benchFactorField: BENCH_FACTOR_FIELD,
  benchPrior: BENCH_PRIOR,
  fail,
...
const {compileLazyWholeTurnPrograms} = createTransitionProgramCompiler({
  ...
  opponentPolicy: OPPONENT_POLICY,
  rootChanceSamples: ROOT_CHANCE_SAMPLES,
  chanceSeedFamily: CHANCE_SEED_FAMILY,
