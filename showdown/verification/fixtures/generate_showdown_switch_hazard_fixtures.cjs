...
if (!showdownRoot) {
  fail("usage: generate_showdown_switch_hazard_fixtures.cjs SHOWDOWN_ROOT");
}
...
const OPPONENT_ITEMS = ["", "Choice Specs"];
const INCOMING_ITEMS = ["", "Heavy-Duty Boots"];
const INCOMING_CASES = [
  {id: "lapras", species: "Lapras", ability: "Shell Armor"},
  {id: "charizard", species: "Charizard", ability: "Blaze"},
];
const HAZARDS = [
  {id: "none", stealthRock: false, spikes: 0},
  {id: "stealth-rock", stealthRock: true, spikes: 0},
  {id: "spikes-1", stealthRock: false, spikes: 1},
  {id: "spikes-2", stealthRock: false, spikes: 2},
  {id: "spikes-3", stealthRock: false, spikes: 3},
...
function incomingSet(spec, item) {
  return {
...
function runFixture({
  ...
  const battle = common.createBattle(
    {preview: false, seed: [79, 83, 89, 97]},
    [
      [
        outgoingSet(),
        incomingSet(incomingCase, incomingItem),
        spareSet(),
      ],
  ...
  const hydroPump = battle.dex.getActiveMove("Hydro Pump");
  ...
  hydroPump.willCrit = false;
  hydroPump.critRatio = 0;
  ...
  const before = {
    ...
    opponent_move_pp: ppFor(opponent, hydroPump.id),
  };
  const context = damageContext(battle, opponent, incoming, hydroPump);
  ...
  battle.damage = function (damage, target, source, effect, instafaint) {
    ...
    const dealt = originalDamage.call(this, damage, target, source, effect, instafaint);
    if (
      activeEffect &&
  ...
  battle.makeChoices("switch 2", "move hydropump");
  battle.prng.random = originalRandom;
  battle.damage = originalDamage;

  const moveLines = battle.log
    .slice(logStart)
    .filter(line => String(line).startsWith("|move|"));
  const opponentAttackExecuted = moveLines.some(
    line => String(line).includes("|p2a:")
  );
  ...
  const fixture = {
    ...
    move_accuracy: hydroPump.accuracy,
    context,
    before,
    after,
...
for (const opponentItem of OPPONENT_ITEMS) {
  for (const incomingItem of INCOMING_ITEMS) {
    for (const incomingCase of INCOMING_CASES) {
...
process.stdout.write(JSON.stringify({
  ...
  source: "pokemon-showdown Battle.makeChoices with preloaded side conditions",
