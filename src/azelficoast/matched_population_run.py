"""Run matched posterior/search treatments on one frozen natural state."""
...
class MatchedPopulationRunError(ValueError):
    """Raised when a natural-state treatment cannot be bound to frozen evidence."""
...
def _state_from_source(
    *,
    ...
    if fixture_id != source.get("fixture_id"):
        raise MatchedPopulationRunError("source and fixture identities disagree")
    if fixture_id != selection.get("fixture_id"):
        raise MatchedPopulationRunError("selection and source fixture identities disagree")
    ...
    *,
    ...
    for treatment in checked_plan["posterior_treatments"]:
        ...
        for depth in checked_plan["depths"]:
            ...
            result["transition_oracle_digest"] = receipts[0][
                "transition_oracle_digest"
            ]
            if receipts[0]["transition_oracle_digest"] != receipts[1][
                "transition_oracle_digest"
            ]:
                raise MatchedPopulationRunError(
                    "paired methods consumed different transition oracles"
                )
...
def matched_cohort(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in selected:
        ...
        if not isinstance(fixture_id, str) or not isinstance(battle_tag, str):
            raise MatchedPopulationRunError("selected row lacks fixture or battle identity")
        rows.append(
            {
                "fixture_id": fixture_id,
                "battle_tag": battle_tag,
